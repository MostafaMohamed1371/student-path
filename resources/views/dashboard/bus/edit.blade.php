...
@section('content')
    ...
    @component('dashboard.partials.shell', ['title' => $title])
        ...
        <section class="card">
            <form method="post" action="{{ $bus ? route('dashboard.bus.update') : route('dashboard.bus.store') }}">
                @csrf
                @if ($bus)
                    @method('put')
                @endif
                ...
                <div style="display:flex;justify-content:flex-end;margin-top:16px;">
                    <button class="btn-primary" type="submit" style="width:auto;padding-inline:16px;">{{ $bus ? __('dashboard.update') : __('dashboard.create') }}</button>
                </div>
            </form>

            @if ($bus)
                <form method="post" action="{{ route('dashboard.bus.destroy') }}" onsubmit="return confirm('{{ __('dashboard.confirm_delete') }}')" style="margin-top:12px;">
                    @csrf
                    @method('delete')
        ...
        <script>
            (function () {
                ...
                if (!picker || !text || !hidden || radios.length === 0) return;
