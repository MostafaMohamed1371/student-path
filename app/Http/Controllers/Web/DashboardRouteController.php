...
class DashboardRouteController extends Controller
{
    ...
    public function create(): View
    {
        ...
        $tripType = old('trip_type', TripType::MORNING_PICKUP->value);
        $drivers = $schoolId > 0 && $tripType !== ''
            ? $this->driversForRouteForm($schoolId, $tripType)
            : collect();
        ...
        return view('dashboard.routes.create', [
            ...
            'tripTypes' => $tripTypes,
            'drivers' => $drivers,
            'formOptionsUrl' => route('dashboard.routes.form_options'),
        ]);
    }
    ...
    public function store(Request $request): RedirectResponse
    {
        ...
        $this->abortUnlessCanMutateSchoolRosterForSchool($schoolId);
        $this->assertDriverMatchesTripType((int) $validated['driver_id'], $schoolId, $validated['trip_type']);
        $this->assertDriverHasNoRouteForTripType((int) $validated['driver_id'], $validated['trip_type']);

        $driver = Driver::query()->with('bus')->findOrFail((int) $validated['driver_id']);
        $validated['shift_period'] = $this->driverShiftResolver->fromTripType($validated['trip_type']);
        $validated['name'] = trim((string) ($validated['name'] ?? '')) !== ''
            ? trim((string) $validated['name'])
            : $this->defaultRouteName($driver, $validated['trip_type']);

        $route = TransportRoute::query()->create($validated);

        $matchResult = $this->routeAssignmentPlanner->assignStudentsAlongRoute($route);

        $successMessage = $matchResult['assigned'] > 0
            ? __('dashboard.route_created_with_students', ['count' => $matchResult['assigned']])
            : __('dashboard.route_created');

        return redirect()
            ->route('dashboard.routes.index', $this->indexQueryParams($schoolId, $validated['trip_type'], (int) $validated['driver_id']))
    ...
    public function edit(TransportRoute $route): View
    {
        ...
        return view('dashboard.routes.edit', [
            ...
            'schools' => $this->schoolsForRosterForm(),
            'tripTypes' => collect(TripType::cases())->map(fn (TripType $t): string => $t->value)->all(),
            'drivers' => $this->driversForRouteForm((int) $route->school_id, (string) $route->trip_type, (int) $route->id),
            'formOptionsUrl' => route('dashboard.routes.form_options'),
        ]);
    }
    ...
    public function update(Request $request, TransportRoute $route): RedirectResponse
    {
        ...
        $this->assertDriverMatchesTripType((int) $validated['driver_id'], $schoolId, $tripType);
        $this->assertDriverHasNoRouteForTripType((int) $validated['driver_id'], $tripType, (int) $route->id);

        $driver = Driver::query()->findOrFail((int) $validated['driver_id']);
        $validated['shift_period'] = $this->driverShiftResolver->fromTripType($validated['trip_type']);
        if (trim((string) ($validated['name'] ?? '')) === '') {
            $validated['name'] = $this->defaultRouteName($driver, $validated['trip_type']);
        }
