...
final class TransportDriverCardBuilder
{
    ...
    public function buildCard(
        ...
        $routeDescription = $this->resolveRouteDescription($bus, $routeKey, $routeBySchoolAndBus);
     ...
     * @param  array<string, string>  $routeBySchoolAndBus
     */
    private function resolveRouteDescription(?Bus $bus, string $routeKey, array $routeBySchoolAndBus): ?string
    {
        ...
        $fromBus = $bus !== null && is_string($bus->route_description) && trim($bus->route_description) !== ''
            ? trim($bus->route_description)
            : null;
        if ($fromBus !== null) {
            return $fromBus;
        }
