...
final class DriverTripModuleService
{
    ...
    public function driverOverview(Driver $driver, ?Carbon $onDay = null): array
    {
        ...
        $tripIdsToday = TripHistory::query()
        ...
        $morningStudents = (int) TripHistoryStudent::query()
            ->whereHas('tripHistory', function ($q) use ($driver, $dayStart, $dayEnd): void {
                $q->where('driver_id', $driver->id)
                    ->whereBetween('start_time', [$dayStart, $dayEnd])
                    ->whereIn('trip_type', [TripType::MORNING_PICKUP->value, TripType::MORNING_RETURN->value]);
            })
            ->count();

        $eveningStudents = (int) TripHistoryStudent::query()
            ->whereHas('tripHistory', function ($q) use ($driver, $dayStart, $dayEnd): void {
                $q->where('driver_id', $driver->id)
                    ->whereBetween('start_time', [$dayStart, $dayEnd])
                    ->whereIn('trip_type', [TripType::EVENING_PICKUP->value, TripType::EVENING_RETURN->value]);
            })
            ->count();
        ...
        return [
            ...
            'available_morning_seats' => max(0, $capacity - $morningStudents),
            'available_evening_seats' => max(0, $capacity - $eveningStudents),
        ];
    }
