...
### Parent transport API (mobile, `/api/v1`)

Parent-app endpoints (wallet, home location, districts/areas, Places proxy, meta, students/trips for guardians, trip requests, absences, notifications, profile, tracking config) live under **`/api/v1`** with optional public OTP on **`/api/v1/auth/send-otp`** and **`/api/v1/auth/verify-otp`**. Full reference and school-scope rules: **`docs/API_V1_PARENT_TRANSPORT.md`**. Postman: **`postman/V1-Parent-Transport.postman_collection.json`**.
...
**URLs (versioned):** Authenticated dashboard lives under **`/dashboard/v1/...`** (e.g. overview `/dashboard/v1`, schools `/dashboard/v1/schools`). Legacy **`/dashboard`** and **`/dashboard/{path}`** (except `/dashboard/v1/...`) **301** to the matching `/dashboard/v1/...` route. Named routes (`dashboard`, `dashboard.schools.index`, …) are unchanged.

Modules:
- Overview (`/dashboard/v1`)
- Schools, Students, Guardians, Drivers, Buses
- Users (`/dashboard/v1/users`) — typically **admin** menu / routes only
- Admin profile (`/dashboard/v1/profile`)
...
- `QI_CARD_BLOCK_DIRECT_RECHARGE` — when `true` (default) and Qi Card is enabled, `POST /api/v1/wallet/recharge` returns **403** so balance is only increased after a verified gateway payment.

**Flow:**

1. App: `POST /api/v1/wallet/payments/qicard/init` (Sanctum) → open `data.form_url` (WebView / browser).
2. Qi Card calls **`POST /api/webhooks/qicard`** and/or redirects the user to **`/api/v1/wallet/payments/qicard/finish`** with `paymentId`.
