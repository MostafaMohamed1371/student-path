...
(function () {
    ...
    if (prefix === 'form') {
        window.iraqLocationFormSetSelection = async function (districtId, areaId, neighborhoodIds) {
            districtId = String(districtId || '');
            areaId = String(areaId || '');
            neighborhoodIds = (neighborhoodIds || []).map(String).filter(function (value) {
                return value !== '';
            });

            districtSelect.value = districtId;
            if (!districtId) {
                await loadAreas('', false, false);
                return;
            }

            await loadAreas(districtId, false, false);
            areaSelect.value = areaId;
            await loadNeighborhoods(districtId, areaId, false);

            if (neighborhoodMultiple) {
                Array.from(neighborhoodSelect.options).forEach(function (option) {
                    option.selected = neighborhoodIds.includes(option.value);
                });
            } else if (neighborhoodIds.length > 0) {
                neighborhoodSelect.value = neighborhoodIds[0];
            }
        };
    }
})();
