...
(function () {
    ...
    function fillForm(data) {
        ...
        if (typeof window.iraqLocationFormSetSelection === 'function') {
            const districtId = data.home_district_id || data.district_id || null;
            const areaId = data.home_area_id || data.area_id || null;
            const neighborhoodIds = data.home_neighborhood_ids || data.neighborhood_ids || [];
            if (districtId || areaId || (Array.isArray(neighborhoodIds) && neighborhoodIds.length > 0)) {
                window.iraqLocationFormSetSelection(districtId, areaId, neighborhoodIds);
            }
        }
    }
