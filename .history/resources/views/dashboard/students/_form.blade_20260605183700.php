            ...
            <select class="input" id="student_form_guardian_id" name="guardian_id" required>
                ...
                @foreach(($guardians ?? collect()) as $guardian)
                    <option
                        ...
                        data-parent-name="{{ (string) $guardian->full_name }}"
...
<section class="form-section">
    ...
    <p class="field-help" style="margin: 0 0 12px;">{{ __('dashboard.student_location_map_help') }}</p>
    <div id="student-location-map" style="height: 260px; border: 1px solid #cbd5e1; border-radius: 10px; margin-bottom: 16px;"></div>
    <div class="form-grid">
        <div>
            <label class="field-label" for="latitude">{{ __('dashboard.latitude') }}</label>
            <input class="input" id="latitude" name="latitude" type="number" step="0.0000001" min="-90" max="90" value="{{ old('latitude', $student->latitude ?? '') }}" />
        </div>
        <div>
            <label class="field-label" for="longitude">{{ __('dashboard.longitude') }}</label>
            <input class="input" id="longitude" name="longitude" type="number" step="0.0000001" min="-180" max="180" value="{{ old('longitude', $student->longitude ?? '') }}" />
        </div>
    </div>
</section>
